...
%3
\sect{作用域}{basic.scope}
\ssect{声明性区域与作用域}{basic.scope.declarative}
...
\ssect{函数原型作用域}{basic.scope.proto}
\paragraph{}
在函数声明或任何除函数定义声明子（11.4）外的声明子中的参数名（如有）有函数原型作
用域，结束于最近包含的函数声明子结束处。

\ssect{函数作用域}{basic.funcscope}
\paragraph{}
标号（9.1）具有函数作用域，且可以在声明它的函数内任何地方使用。只有标号具有函数
作用域。

\ssect{命名空间作用域}{basic.scope.namespace}
\paragraph{}
...
\ssect{枚举作用域}{basic.scope.enum}
\paragraph{}
作用域枚举子的名字具有枚举作用域。其潜在作用域始于其声明点，终于
\textit{enum-specifiers}结束处。

\ssect{模板参数作用域}{basic.scope.temp}
\paragraph{}
...
\ssect{未限定名查询}{basic.lookup.unqual}
\paragraph{}
...
\ssect{参数依赖名查询}{basic.lookup.argdep}
\paragraph{}
...
\mbox{\qquad\textit{nested-name-specifier\tsub{opt}
...
\ssect{详尽类型说明符}{basic.lookup.elab}
\paragraph{}
...
\ssect{类成员访问}{basic.lookup.classref}
\paragraph{}
...
\ssect{Using指令和命名空间别名}{basic.lookup.udir}
\paragraph{}
...
%6
\sect{程序启动与结束}{basic.start}
\ssect{main函数}{basic.start.main}
\paragraph{}
...
\ssect{静态初始化}{basic.start.static}
\paragraph{}
...
\ssect{非局部变量动态初始化}{basic.start.dynamic}
\paragraph{}
...
\sssect{分配函数}{basic.stc.dynamic.allocation}
\paragraph{}
...
\sssect{回收函数}{basic.stc.dynamic.deallocation}
\paragraph{}
...
\sssect{安全派生指针}{basic.stc.dynamic.safety}
\paragraph{}
可追踪指针对象指：
\begin{enumerate}
...
\ssect{子对象存储期}{basic.stc.inherit}
\paragraph{}
子对象和引用成员存储期为其完整对象的存储期（4.5）。

%8
\sect{对象生命期}{basic.life}
\paragraph{}
...
\ssect{CV限定符}{basic.type.qualifier}
\paragraph{}
