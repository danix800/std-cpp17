...
\synsym{lambda-expression}
  \synprd{\textit{lambda-introducer lambda-declarator\tsub{opt}
    compound-statement}}
\synsym{lambda-introducer}
  \synprd{\texttt{[} \textit{lambda-capture\tsub{opt}} \texttt{]}}
\synsym{lambda-declarator}
  \synprd{\texttt{(} \textit{parameter-declaration-clause} \texttt{)}
    \textit{decl-specifier-seq\tsub{opt}}}
  \synprd[]{\qquad \textit{noexcept-specifier\tsub{opt}
            attribute-specifier-seq\tsub{opt} trailing-return-type\tsub{opt}}}
